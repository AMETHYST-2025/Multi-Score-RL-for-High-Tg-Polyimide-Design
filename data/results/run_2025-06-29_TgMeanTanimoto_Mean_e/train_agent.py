...
from reinvent_benchmarking.model import RNN
from reinvent_benchmarking.data_structs_n import Vocabulary, Experience
from reinvent_benchmarking.scoring_functions import get_scoring_function
from reinvent_benchmarking.utils import Variable, seq_to_smiles, fraction_valid_smiles, unique
...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    for step in range(n_steps):
        #scoring_function = get_scoring_function(scoring_function="tanimoto", step=step, n_steps=n_steps, num_processes=num_processes) #"Tg_score", "ProgressiveTgScorer", "tanimoto"
        # Current generation
        ...
        #fitness = scoring_function(smiles, step, n_steps) # score en fonction de step
        ...
        n = min(10, len(smiles), len(agent_likelihood), len(prior_likelihood), len(augmented_likelihood), len(score), len(fitness))
        for i in range(n):
    ...
    copyfile('models_jupyter/reinvent_benchmarking/train_agent.py', os.path.join(save_dir, "train_agent.py"))
